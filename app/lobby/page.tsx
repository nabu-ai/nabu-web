...
export default function LobbyPage() {
  ...
  const serverToken = useUserStore().getLoginData()?.accessToken;
  ...
  const handleJoin = async () => {
    ...
    if(!nonVerbal){
      prepareStreaming(preferredLanguage, serverToken);
    }
  ...
  const fetchGuestToken = async () => {
    ...
    const response = await fetch(
      `${NABU_MEETING_HOST}/api/guest/meetings/token?channel=${meetingInfo.meetingId.trim()}&uid=${meetingInfo.participants?.[0]?.name}`,
      {
        method: "GET",
        headers: {
          "X-Tenant-Id": meetingInfo.tenantId,
        }
      }
    );
    const { token, appId } = await response.json();
  ...
  return <MediaPermissionPrompt onPermissionChange={setPermissionStatus}/>;
